...
def main(args):
    ...
    def custom_text_forward(*args, **kwargs):
        if isinstance(kwargs['input_ids'], dict) and "inj_embedding" in kwargs['input_ids']:
            return inj_forward_text(*args, **kwargs)
        else:
            return orig_text_forward(*args, **kwargs)

    text_encoder.text_model.forward = custom_text_forward
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                latents = vae.encode(batch["pixel_values_vae_gt"].to(dtype=weight_dtype)).latent_dist.sample() # the input pixel_values should be [-1, 1]
                ...
                with torch.no_grad():
                    # 提取图像特征
                    image_features = clip_image_encoder(batch["pixel_values_vae_gt"]).last_hidden_state
                    # 通过mapper生成嵌入
                ...
                text_input = {
                    ...
                    "inj_index": torch.zeros(batch["input_ids"].size(0)).long().to(accelerator.device)  # 假设固定注入位置
                }
